    ...
    <div className="max-w-4xl mx-auto">
      ...
      {allPosts.length === 0 ? (
        <div className="text-center py-12">
          <div className="terminal:text-terminal-accent blue:text-bluef-accent light:text-light-accent font-mono text-lg mb-4">
            $ find . -name "*.md" -type f
          </div>
          <p className="terminal:text-terminal-text blue:text-bluef-text light:text-light-text opacity-60">
          ...
          {allPosts.map(({ slug, date, title }) => (
            ...
            >
              <div className="flex items-start justify-between mb-3">
                <h2 className="text-xl font-bold terminal:text-terminal-text blue:text-bluef-text light:text-light-text font-mono">
                  <Link
                    href={`/blog/${slug}`}
                    className="terminal:hover:text-terminal-accent blue:hover:text-bluef-accent light:hover:text-light-accent transition-colors"
                  >
                    {title}
                  </Link>
                </h2>
                <time className="text-sm terminal:text-terminal-accent blue:text-bluef-accent light:text-light-accent font-mono opacity-80">
                  {date}
                </time>
              </div>
      ...
      <div className="mt-12 pt-8 border-t terminal:border-terminal-accent/30 blue:border-bluef-accent/30 light:border-gray-300">
        <div className="text-center font-mono terminal:text-terminal-text blue:text-bluef-text light:text-light-text opacity-70">
